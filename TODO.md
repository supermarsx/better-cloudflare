...
- Passkey/vault endpoint authorization: add a real auth check (Cloudflare token verification or a server-side session) instead of only checking for the presence of headers.
- Passkey revoke authorization: either allow per-key revocation without admin or update UI/API to require an admin token consistently.
