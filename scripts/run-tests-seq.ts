...
for (const f of files) {
  ...
  const res = spawnSync('npx', ['tsx', '--test', full], { stdio: 'inherit', shell: true, windowsHide: false });
  if (res.status !== 0) {
    console.error(`Test ${f} failed or returned non-zero exit status ${res.status}. Stopping.`);
