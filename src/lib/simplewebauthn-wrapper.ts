...
export const verifyRegistrationResponse = async (
  ...
  if (!mod || !mod.verifyRegistrationResponse)
    throw new Error("@simplewebauthn/server not available");
  return mod.verifyRegistrationResponse(
    ...args,
  ) as Promise<VerifyRegistrationResult>;
};
...
export const verifyAuthenticationResponse = async (
  ...
  const mod = loadSwauthSync();
  if (!mod || !mod.verifyAuthenticationResponse)
    throw new Error("@simplewebauthn/server not available");
  return mod.verifyAuthenticationResponse(
    ...args,
  ) as Promise<VerifyAuthenticationResult>;
};
...
export const generateRegistrationOptions = (
  ...
  const mod = loadSwauthSync();
  if (mod && mod.generateRegistrationOptions)
    return mod.generateRegistrationOptions(opts);
  ...
  const rpName = (opts as Record<string, unknown>)["rpName"] as
    | string
    | undefined;
...
export const generateAuthenticationOptions = (
  ...
  const mod = loadSwauthSync();
  if (mod && mod.generateAuthenticationOptions)
    return mod.generateAuthenticationOptions(opts);
  const globalCrypto =
    typeof globalThis !== "undefined"
      ? (globalThis as { crypto?: { randomUUID?: () => string } }).crypto
      : undefined;
  ...
  const allowCredentials =
    (opts as Record<string, unknown>)["allowCredentials"] ?? [];
  const rpId =
    (opts as Record<string, unknown>)["rpID"] ??
    (opts as Record<string, unknown>)["rpId"];
