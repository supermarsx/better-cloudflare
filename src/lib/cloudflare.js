...
export class CloudflareAPI {
     ...
     */
    constructor(apiKey, baseUrl = getEnv('CLOUDFLARE_API_BASE', 'VITE_CLOUDFLARE_API_BASE', DEFAULT_CLOUDFLARE_API_BASE), email) {
     ...
     */
    buildRecordParams(zoneId, record) {
        const params = {
            zone_id: zoneId,
            type: record.type,
            name: record.name,
    ...
    async getDNSRecords(zoneId, signal) {
        if (DEBUG)
            console.debug('getDNSRecords', { zoneId });
    ...
    async createDNSRecord(zoneId, record, signal) {
        if (DEBUG)
            console.debug('createDNSRecord', { zoneId, record });
        this.debugRequest(`/zones/${zoneId}/dns_records`, {
            method: 'POST',
            body: record,
        });
        const params = this.buildRecordParams(zoneId, record);
        const result = (await this.client.dns.records.create(params, { signal }));
    ...
    async updateDNSRecord(zoneId, recordId, record, signal) {
        if (DEBUG)
            console.debug('updateDNSRecord', { zoneId, recordId, record });
        this.debugRequest(`/zones/${zoneId}/dns_records/${recordId}`, {
            method: 'PUT',
            body: record,
        });
        const params = this.buildRecordParams(zoneId, record);
        const result = (await this.client.dns.records.update(recordId, params, { signal }));
        this.debugResponse(result);
        return result;
    }
    /**
    ...
    async deleteDNSRecord(zoneId, recordId, signal) {
        if (DEBUG)
            console.debug('deleteDNSRecord', { zoneId, recordId });
