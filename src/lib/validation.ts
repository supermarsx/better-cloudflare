...
export const dnsRecordSchema = z.object({
  ...
  if (val.type === 'MX') {
    if (typeof val.priority !== 'number' || !Number.isInteger(val.priority)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'MX records must include an integer priority' });
    }
    // MX content should be a simple hostname — no spaces
    if (typeof val.content !== 'string' || val.content.trim().length === 0 || /\s+/.test(val.content)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'MX content must be a non-empty hostname with no spaces' });
    }
  }
  // A/AAAA content validation
  if (val.type === 'A') {
    if (typeof val.content !== 'string' || net.isIP(val.content) !== 4) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'A record content must be a valid IPv4 address' });
    }
  }
  if (val.type === 'AAAA') {
    if (typeof val.content !== 'string' || net.isIP(val.content) !== 6) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'AAAA record content must be a valid IPv6 address' });
    }
  }
  ...
  if (val.type === 'SRV') {
    ...
    if (!srvRe.test(String(val.content))) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'SRV content must be: "priority weight port target"' });
    }
  }
  ...
  if (val.type === 'TLSA') {
    ...
    if (!tlsaRe.test(String(val.content))) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'TLSA content must be: "usage selector matching-type data"' });
    }
  }
  ...
  if (val.type === 'SSHFP') {
    ...
    if (!sshfpRe.test(String(val.content))) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'SSHFP content must be: "algorithm fptype fingerprint"' });
    }
  }
  ...
  if (val.type === 'NAPTR') {
    const splitNaptrTokens = (s: string) => {
      ...
      for (let i = 0; i < s.length; i++) {
        ...
        if (ch === ' ' && !inQuote) { if (current.trim().length > 0) { tokens.push(current.trim()); current = ''; } continue; }
        current += ch;
      }
    ...
    if (parts.length < 6) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR content must be: "order preference flags service regexp replacement"' });
    } else {
      if (!/^\d+$/.test(parts[0])) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR order must be an integer' });
      }
      if (!/^\d+$/.test(parts[1])) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR preference must be an integer' });
      }
      // flags should be non-empty and single token
      if (!parts[2] || typeof parts[2] !== 'string' || parts[2].trim() === '') {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR flags must be a non-empty token' });
      }
      // service should be a non-empty token and not contain spaces
      if (!parts[3] || typeof parts[3] !== 'string' || parts[3].trim() === '' || /\s/.test(parts[3])) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR service must be a non-empty token' });
      }
      // regexp should be a quoted string or slash-delimited regex
      if (!parts[4] || typeof parts[4] !== 'string' || parts[4].trim() === '') {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR regexp must be non-empty' });
      }
      // replacement should be a domain (or @)
      if (!parts[5] || typeof parts[5] !== 'string' || parts[5].trim() === '') {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'NAPTR replacement must be a non-empty token' });
      }
    }
  }
  ...
  if (['CNAME', 'NS', 'PTR', 'ALIAS', 'ANAME'].includes(String(val.type))) {
    ...
    if (!hostnameRe.test(String(val.content))) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: `${val.type} content must be a valid hostname` });
    }
  }
