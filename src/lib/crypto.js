...
function isValidAlgorithm(alg) {
    return ENCRYPTION_ALGORITHMS.includes(alg);
}
/**
...
export class CryptoManager {
    ...
    async deriveKey(password, salt) {
        ...
        return crypto.subtle.deriveKey({
            ...
            hash: 'SHA-256'
        }, keyMaterial, { name: this.config.algorithm, length: this.config.keyLength }, false, ['encrypt', 'decrypt']);
    }
    /**
    ...
    async encrypt(data, password) {
        ...
        const encrypted = await crypto.subtle.encrypt({ name: this.config.algorithm, iv: iv }, key, encoder.encode(data));
        return {
    ...
    async decrypt(encryptedData, salt, iv, password) {
        ...
        const decrypted = await crypto.subtle.decrypt({ name: this.config.algorithm, iv: this.base64ToArrayBuffer(iv) }, key, this.base64ToArrayBuffer(encryptedData));
