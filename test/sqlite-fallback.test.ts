  ...
  Database: function (file?: string) {
    ...
    this.run = (sql: string, params?: unknown[] | unknown, cb?: unknown) => {
      ...
      if (/DELETE\s+FROM\s+CREDENTIALS/i.test(sql)) {
        ...
        dbState.credentials = dbState.credentials.filter(
          (r: DBRow) => !(r.id === id && r.credential_id === cid),
        );
        dbState.lastID += 1;
        if (cb)
          cb.call(
            {
              lastID: dbState.lastID,
    ...
    this.get = (sql: string, params?: unknown[] | unknown, cb?: unknown) => {
      ...
      if (m) {
        ...
        const id = p[0];
        dbState.tables = dbState.tables ?? {};
        const row = (dbState.tables?.[tbl] || []).find(
          (r: DBRow) => r.id === id,
        );
        if (cb) cb(null, row);
        return;
      ...
      if (
        s.startsWith("SELECT CREDENTIAL_ID") ||
        s.includes("FROM CREDENTIALS")
      ) {
        const id = p[0];
        const row = id
          ? dbState.credentials.find((r: DBRow) => r.id === id)
          : dbState.credentials[0];
        if (cb) cb(null, row);
        return;
      }
    ...
    this.all = (sql: string, params?: unknown[] | unknown, cb?: unknown) => {
      ...
      if (
        ...
        const id =
          Array.isArray(params) && (params as unknown[]).length
            ? (params as unknown[])[0]
            : undefined;
        const rows = id
          ? dbState.credentials.filter((r: DBRow) => r.id === id)
          : dbState.credentials;
        if (cb) cb(null, rows);
        return;
      }
      if (
        s.startsWith("SELECT ID, ACTOR, OPERATION") ||
        s.includes("FROM AUDIT_LOG")
      ) {
...
test("SqliteCredentialStore works with injected sqlite3 wrapper", async () => {
  ...
  if ((wrapper as unknown as { close?: () => Promise<void> }).close)
    await (wrapper as unknown as { close?: () => Promise<void> }).close?.();
  try {
    fs.unlinkSync(tmp);
  } catch {
    /* ignore cleanup errors */
