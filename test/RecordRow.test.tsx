...
const sample: DNSRecord = {
  id: '1', type: 'A', name: 'www', content: '1.2.3.4', ttl: 300, zone_id: 'z', zone_name: 'example.com', created_on: '', modified_on: ''
};

test('RecordRow renders display mode when not editing', () => {
  const tree = create(
    React.createElement(RecordRow, { record: sample, isEditing: false, onEdit: () => {}, onSave: () => {}, onCancel: () => {}, onDelete: () => {} })
  );
  const root = tree.root;
  // display mode should contain the record name
  const text = root.findAllByType('div').map((n) => (n.children && typeof n.children[0] === 'string' ? (n.children[0] as string) : null)).filter(Boolean);
  assert.ok(text.length > 0);
});

test('RecordRow edit flow calls onSave with updated record', () => {
  let saved: DNSRecord | null = null;
  const onSave = (r: DNSRecord) => { saved = r; };
  const tree = create(
    React.createElement(RecordRow, { record: sample, isEditing: true, onEdit: () => {}, onSave, onCancel: () => {}, onDelete: () => {} })
  );
  const root = tree.root;
