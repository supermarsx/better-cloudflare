...
type MockReq = Partial<Request> & { header: (name: string) => string | undefined };

function makeReq(body: unknown, params: Record<string, unknown>, headers?: Record<string, string>): MockReq {
  return {
...
function makeRes() {
  ...
  const res: Partial<Response> = {
    ...
    json(data: unknown) { jsonData = data; return this as Response; },
  };
...
test('RBAC: admin token or user role is required for admin endpoints', async () => {
  ...
  const store = createCredentialStore() as ReturnType<typeof createCredentialStore>;
  ...
  await ServerAPI.createUser()(reqCreate as unknown as Request, createdRes.res as Response);
  ...
  const arRes = makeRes();
  try {
    await ServerAPI.getAuditEntries()(arReq as unknown as Request, arRes.res as Response);
  ...
  await ServerAPI.getAuditEntries()(arReq2 as unknown as Request, arRes2.res as Response);
  ...
  await new Promise<void>((resolve, reject) => {
    isAdmin(mwReq as unknown as Request, mwRes.res as Response, (err?: unknown) => {
      if (err) {
        reject(err);
      } else {
        called = true;
