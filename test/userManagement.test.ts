...
test('admin user creation and roles update (sqlite)', async () => {
  ...
  const req = { body: { id: 'u1', email: 'u1@example.com', roles: ['user'] }, header(name: string) { return name === 'x-admin-token' ? 'admin-token' : undefined; } } as unknown as Request;
  function makeResponse() {
    ...
    return { res: resObj as Response, get status() { return statusCode; }, get data() { return jsonData; } };
  }
  ...
  const reqGet = { params: { id: 'u1' }, header() { return 'admin-token'; } } as unknown as Request;
  ...
  const reqUpdate = { params: { id: 'u1' }, body: { roles: ['admin'] }, header() { return 'admin-token'; } } as unknown as Request;
