...
An unfiltered diagnostic scan using an empty config override reports 18
affected Rust packages: 16 maintenance-only notices, one unsoundness advisory,
and one unpatched vulnerability. OSV lists a nominal fixed version for `glib`,
but it is not solver-reachable in the current Tauri stack: `gtk 0.18` requires
`glib ^0.18`. Replacing that dependency family requires an upstream Tauri/GTK
migration rather than a safe lockfile update.

The unpatched vulnerability is `RUSTSEC-2023-0071` (`rsa@0.9.10`), which entered
the graph with the WebAuthn relying-party work. Unlike every other entry it has
no patched release in any line, so it cannot be upgraded away; it is waived on
reachability with a deliberately shorter expiry. See the register below.
...
expiry date: 2026-10-30 for every entry **except `RUSTSEC-2023-0071`, which
expires earlier, on 2026-10-02**.

Every other entry waives an _unmaintained_ or _unsound_ package where no patched
release can exist without an upstream runtime migration, so the review date is
the only thing that moves them. `RUSTSEC-2023-0071` is different: it is a live
vulnerability with a plausible upstream resolution, so it is deliberately given a
shorter fuse to force re-examination rather than letting it drift to the common
date. It must also be re-verified on **every `webauthn-rs` or `crypto-glue`
bump**, because the justification rests on current call paths rather than on the
vulnerable code being absent.

It clears on one of exactly two conditions - RustSec publishing a fixed `rsa`
version, or `webauthn-rs`/`crypto-glue` dropping `rsa`. **Moving to `rsa` 0.10 is
not one of them**: the advisory's only range event is `{"introduced": "0.0.0-0"}`
with no `fixed` event, so `0.10.0-rc.18` is affected exactly as `0.9.10` is, and
discharging on it would mark the entry handled while it is not.

| ID                  | Package                    | Class        | Rationale and upstream reference                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------- | -------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RUSTSEC-2023-0071` | `rsa@0.9.10`               | unpatched    | Marvin timing side-channel. Reaches the graph transitively via `webauthn-rs 0.6.1-dev -> crypto-glue 0.1.15`. No patched release exists in any line (`patched = []`; upstream [RustCrypto/RSA#626](https://github.com/RustCrypto/RSA/issues/626) is open), so it cannot be upgraded or patched away. Marvin recovers a private key by timing **private-key** operations; a WebAuthn relying party holds no RSA private key and decrypts nothing — `webauthn-rs-core` touches only `RS256PublicKey::new`, `RS256Signature::try_from` and `RS256VerifyingKey::new` (`crypto.rs:498/559/561`), and the crate's only `Oaep` occurrence is a commented-out TPM constant. **Linked but uncalled, not absent**: `crypto-glue`'s `rsa` module (inline at `lib.rs:482`, not a `src/rsa.rs` file) publicly exposes `RS256PrivateKey`, `RS256SigningKey`, `new_key()` and `oaep_sha256_encrypt`, none test-gated. Expires **2026-10-02**, earlier than every other entry. [Advisory](https://rustsec.org/advisories/RUSTSEC-2023-0071.html) |
